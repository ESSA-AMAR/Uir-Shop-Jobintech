...
export default function ProducTextile() {
  ...
  useEffect(() => {
    axios
      ...
      .then(function (response) {
        ...
        const filteredData = response.data.filter(item => item.categoryID === 1);
...
<div className='flex justify-end'>
      ...
      <div className='bg-[#FFFF] w-full py-4 px-5 bg5 my-10 relative rounded-3xl '>
        <div className='flex items-center gap-3 absolute top-[-80px]'>
          <img src={img2} alt="" className='rounded-full w-[150px]' />
          <h2 className='text-[#183761] font-bold text-[42px]'>Textile</h2>
        </div>
